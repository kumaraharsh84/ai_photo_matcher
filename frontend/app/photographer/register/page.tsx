...
export default function RegisterPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true);
    try {
      await register(name.trim(), cleanEmail(email), password);
    } catch (err) {
    ...
    <main className="grid min-h-screen place-items-center bg-mist px-4 py-12">
      <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-8 shadow-soft">
        <div className="mb-6 flex flex-wrap gap-4 text-sm font-semibold">
          <Link href="/photographer/login" className="inline-flex items-center gap-2 text-slate-500 hover:text-ink">
            <ArrowLeft size={16} />
            Login
          </Link>
        ...
        <h1 className="text-3xl font-bold">Create Photographer Account</h1>
        <p className="mt-2 text-sm text-slate-500">Start building shareable event galleries.</p>
        <form onSubmit={submit} className="mt-8 space-y-4">
          <label className="block">
            <span className="text-sm font-semibold">Name</span>
            <input className="focus-ring mt-1 w-full rounded-lg border border-slate-300 px-4 py-3" value={name} onChange={(e) => setName(e.target.value)} required />
          </label>
          <label className="block">
          ...
          <label className="block">
            ...
            <input className="focus-ring mt-1 w-full rounded-lg border border-slate-300 px-4 py-3" type="password" minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} required />
          </label>
        ...
        <p className="mt-6 text-center text-sm text-slate-600">
          Already have an account? <Link className="font-semibold text-rosewood" href="/photographer/login">Login</Link>
        </p>
      </div>
    </main>
  );
